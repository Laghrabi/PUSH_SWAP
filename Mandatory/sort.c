...
t_stack	*only_3(t_stack **stackA, t_stack **stackB, int size)
{
	...
	if ((*stackA)->next->next != NULL)
	{
		if ((*stackA)->data > (*stackA)->next->data
			...
			ra(stackA);
		else if ((*stackA)->next->data > (*stackA)->data
 ...
 *
 * Sorts stackA by moving the smallest element to stackB,
	sorting the remaining 3 elements,
 ...
 * @stackB: Double pointer to stackB (used as temporary storage).
 *
 * Sorts stackA by moving the two smallest elements to stackB,
	sorting the remaining 3 elements,
 ...
 * Depending on size, sorts stackA using specialized functions (for sizes <= 5)
	...
	* or a more complex algorithm (for larger sizes).
    Returns the sorted stackA or NULL.
 */
